...
However, Classroom 50 and Git Classroom differ significantly in how they handle "backend" operations and students' permissions.
...
(Classroom 50 uses a similar workflow-as-a-backend design for some teacher-facing operations like collecting scores and re-running autograders, but not for student-facing operations like accepting assignments.)

Moreover, Git Classroom does not require access to any students' personal GitHub resources; rather, it requires access only to the classroom's organizational resources, in which it reads assignment templates, generates assignment repositories, and sends repository invites to students. It conducts an OAuth flow, but only to retrieve the student's username for purposes of looking up their GitHub ID, naming their assignment repositories, and sending them invites. The GitHub App through which students conduct the OAuth flow (the Workflow Dispatch App) is not (and *cannot*) be installed on students' GitHub accounts---it's entirely private to the classroom's organization.

One downside to Git Classroom's design is that it uses GitHub's cloud-hosted Actions runners by default, so backend operations can be somewhat slow, especially when the runners are experiencing heavy load. This can be mostly overcome with self-hosted runners and additional manual configuration. However, even with self-hosted runners, most backend operations will still suffer from some minor delay due to the asynchronous nature of GitHub Actions workflows.
