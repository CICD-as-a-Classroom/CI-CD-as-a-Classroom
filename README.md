...
- Since students must be added to the classroom's GitHub Organization in order to be a part of the student team, the organization must be carefully hardened to avoid giving students more permissions than necessary. Some hardening operations cannot be automated by the GitHub REST API and must be conducted manually by the instructor once per organization.
- Students are generally given admin access to their assignment repositories since their user access tokens are used to create them. (Though, the permissions of a repository admin can be adjusted as a part of the aforementioned organization hardening.)
...
One downside to Git Classroom's design is that it uses GitHub-hosted Actions runners by default, so backend operations (e.g., accepting an assignment) can be somewhat slow, and instructors teaching large classes many run up against their limits for free GitHub Actions minutes. These issues can be mostly overcome with self-hosted runners as explained [above](#Self-hosted runners). However, due to the asynchronous, event-driven nature of Actions workflows, backend operations may still be somewhat slow even with self-hosted runners (essentially, Actions workflows simply aren't designed for handling frequent user-facing operations; Git Classroom recognizes this but uses them anyways as a workaround to keep things free, serverless, and easy for instructors to self-host, all while keeping some operations centralized and protected).
