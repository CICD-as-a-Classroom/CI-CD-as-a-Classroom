...
Note: Currently, these repository invites only grant students `push` (`write`) access to their assignment repositories&mdash;not `admin` access. This means that students cannot currently add each other as collaborators to their assignment repositories.
